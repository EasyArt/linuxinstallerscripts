...
# Ask whether to install the agent
if whiptail \
    --title "Beszel Agent" \
    --yesno "Would you like to install the Beszel Agent on this host?" \
    10 60; then

    docker volume inspect beszel_agent_data >/dev/null 2>&1 || docker volume create beszel_agent_data >/dev/null
    ...
    whiptail \
        ...
        --msgbox "Beszel Agent has been installed successfully.\n\nIMPORTANT:\nOpen your Docker management interface and replace the TOKEN and KEY environment variables with the values generated by your Beszel Hub before using the agent." \
        15 80
fi

whiptail \
    --title "Installation Complete" \
    --msgbox "Beszel installation completed successfully." \
    10 60
